...
def annotate(p, user):
    json_path = "src/puck/cli/annotations_" + user + ".json"
    with open(json_path , "r") as json_file:
        file_data = json.loads(json_file.read())
    for file_path in sorted(list(p.glob('images_miniset/*/*/*/*/*[0].jpg'))):
        annotate_this_img = True
        file_path_str = str(file_path)
        if file_path_str not in file_data:
            file_data.update({file_path_str:run(file_path_str, user)})
